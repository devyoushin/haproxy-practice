...
usage() {
  cat <<'EOF'
Usage:
  deploy-haproxy-config.sh --inventory <file> --config-root <dir> --user <ssh-user>
...
while read -r host group node_id _rest; do
  ...
  if [ ! -f "${work_dir}/config/haproxy.cfg" ]; then
    echo "Missing haproxy.cfg for ${host}" >&2
    exit 1
  fi

  tar -C "${work_dir}/config" -czf "${work_dir}/haproxy-config.tgz" .
  scp ${SSH_OPTS} "${work_dir}/haproxy-config.tgz" "${USER_NAME}@${host}:/tmp/haproxy-config.tgz"
  ssh ${SSH_OPTS} "${USER_NAME}@${host}" 'set -euo pipefail
    sudo rm -rf /tmp/haproxy-config-next
    sudo mkdir -p /tmp/haproxy-config-next
    sudo tar -xzf /tmp/haproxy-config.tgz -C /tmp/haproxy-config-next
    sudo haproxy -c -f /tmp/haproxy-config-next/haproxy.cfg
    if [ -d /etc/haproxy ]; then
      sudo cp -a /etc/haproxy "/etc/haproxy.backup.$(date +%Y%m%d%H%M%S)"
    fi
    sudo mkdir -p /etc/haproxy
    sudo cp -a /tmp/haproxy-config-next/. /etc/haproxy/
