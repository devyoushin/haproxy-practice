...
| `deploy-haproxy-config.sh` | 서버별 HAProxy 설정 bundle을 배포하고 검증 후 reload하는 도구 |
...
```text
10.0.1.10 edge haproxy-edge-a
```

배포 시 아래 디렉터리를 순서대로 합쳐 `/etc/haproxy`에 반영합니다.

```text
config-bundle/common/
config-bundle/groups/edge/
config-bundle/hosts/haproxy-edge-a/
```
...
vi ops/bulk-install/config-bundle.prod/groups/edge/haproxy.cfg
```

한 대에 먼저 설정을 배포합니다.

```bash
ops/bulk-install/deploy-haproxy-config.sh \
  --inventory <(printf '10.0.1.10 edge haproxy-edge-a\n') \
  --user ec2-user \
...
1. `/tmp/haproxy-config-next`에 새 설정 압축 해제
2. `haproxy -c -f /tmp/haproxy-config-next/haproxy.cfg`로 사전 검증
3. 기존 `/etc/haproxy`를 `/etc/haproxy.backup.<timestamp>`로 백업
4. 새 설정을 `/etc/haproxy`에 복사
5. `haproxy -c -f /etc/haproxy/haproxy.cfg` 재검증
6. `systemctl reload-or-restart haproxy`
